...
void proximaCarta(void) // muda a carta atual do monte
{
	if (atual == NULL)
		atual = monte;
	else
...
int moveMonteBaixo(int j)
{
	...
	{
		while (n_ant->prox != atual) // enquanto o proximo da carta anterior a atual nao for a carta atual
		{
			n_ant = n_ant->prox;
		} // n_ant aponta para a carta anterior a atual
	}

	if (n1 != NULL)
	{					  // se a pilha de baixo nao estiver vazia
		n1->prox = atual; // o proximo da ultima carta da pilha de baixo aponta para a carta atual do monte
	}
	else
	{
		if (atual->valor != 13)
	...
	if (n_ant != NULL)			   // se a carta anterior a atual nao for a primeira carta do monte
		n_ant->prox = atual->prox; // a carta anterior a atual aponta para a carta seguinte a atual
	else
		monte = atual->prox;
	n2 = atual;
	atual = atual->prox;
